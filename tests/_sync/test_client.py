...
from harvest_forecast._sync.client import SyncForecastClient
...
def test_sends_forecast_account_id_header(client_kwargs: dict[str, object]) -> None:
    with respx.mock() as mock:
        ...
        with SyncForecastClient(**client_kwargs) as client:
            client.list_people()
...
def test_sends_authorization_bearer_header(client_kwargs: dict[str, object]) -> None:
    with respx.mock() as mock:
        route = mock.route(method="GET", url__startswith=f"{BASE}/people").mock(
            return_value=httpx.Response(200, json={"people": [_person_data()], "links": {}}),
        )
...
def test_sends_user_agent_header(client_kwargs: dict[str, object]) -> None:
    with respx.mock() as mock:
        route = mock.route(method="GET", url__startswith=f"{BASE}/people").mock(
            return_value=httpx.Response(200, json={"people": [_person_data()], "links": {}}),
        )
...
def test_list_people(client_kwargs: dict[str, object]) -> None:
    with respx.mock() as mock:
...
def test_list_clients(client_kwargs: dict[str, object]) -> None:
    with respx.mock() as mock:
...
def test_list_milestones(client_kwargs: dict[str, object]) -> None:
    with respx.mock() as mock:
...
def test_list_projects(client_kwargs: dict[str, object]) -> None:
    with respx.mock() as mock:
...
def test_list_roles(client_kwargs: dict[str, object]) -> None:
    with respx.mock() as mock:
...
def test_list_placeholders(client_kwargs: dict[str, object]) -> None:
    with respx.mock() as mock:
...
def test_list_repeated_assignment_sets(client_kwargs: dict[str, object]) -> None:
    with respx.mock() as mock:
        ...
        with SyncForecastClient(**client_kwargs) as client:
            sets = client.list_repeated_assignment_sets()
    assert len(sets) == 1
    assert sets[0].id == 1


def test_list_user_connections(client_kwargs: dict[str, object]) -> None:
    with respx.mock() as mock:
        ...
        with SyncForecastClient(**client_kwargs) as client:
            conns = client.list_user_connections()
    assert len(conns) == 1
...
def test_list_assignments_with_date_range(client_kwargs: dict[str, object]) -> None:
    with respx.mock() as mock:
...
def test_list_assignments_chunks_large_date_range(
...
def test_list_assignments_deduplicates_by_id(
    client_kwargs: dict[str, object],
) -> None:
    page1 = {"assignments": [_assignment_data(1), _assignment_data(2)], "links": {}}
...
def test_list_assignments_enforces_2520_day_cap(
    client_kwargs: dict[str, object],
) -> None:
    with respx.mock() as mock:
        route = mock.route(method="GET", url__startswith=f"{BASE}/assignments").mock(
...
def test_list_assignments_rejects_inverted_dates(
    client_kwargs: dict[str, object],
) -> None:
    with SyncForecastClient(**client_kwargs) as client:
...
def test_list_assignments_requires_date_filter(
    client_kwargs: dict[str, object],
) -> None:
    with SyncForecastClient(**client_kwargs) as client:
...
def test_list_assignments_with_filter_params(
    client_kwargs: dict[str, object],
) -> None:
    with respx.mock() as mock:
        route = mock.route(method="GET", url__startswith=f"{BASE}/assignments").mock(
...
def test_get_person(client_kwargs: dict[str, object]) -> None:
    with respx.mock() as mock:
...
def test_get_person_rejects_zero_id(client_kwargs: dict[str, object]) -> None:
    with SyncForecastClient(**client_kwargs) as client:
        with pytest.raises(ValueError, match="id must be >= 1"):
            client.get_person(0)


def test_get_project(client_kwargs: dict[str, object]) -> None:
    with respx.mock() as mock:
...
def test_get_person_404(client_kwargs: dict[str, object]) -> None:
    with respx.mock() as mock:
...
def test_get_placeholder(client_kwargs: dict[str, object]) -> None:
    with respx.mock() as mock:
        ...
        with SyncForecastClient(**client_kwargs) as client:
            ph = client.get_placeholder(7)
    assert ph.id == 7
    assert ph.name == "TBD"


def test_get_role(client_kwargs: dict[str, object]) -> None:
    with respx.mock() as mock:
        mock.route(method="GET", url__startswith=f"{BASE}/roles/3").mock(
            return_value=httpx.Response(200, json={"role": {"id": 3, "name": "Dev"}}),
        )
        with SyncForecastClient(**client_kwargs) as client:
            role = client.get_role(3)
    assert role.id == 3
    assert role.name == "Dev"


def test_get_repeated_assignment_set(client_kwargs: dict[str, object]) -> None:
    with respx.mock() as mock:
        ...
        with SyncForecastClient(**client_kwargs) as client:
            ras = client.get_repeated_assignment_set(5)
    assert ras.id == 5
...
def test_create_assignment(client_kwargs: dict[str, object]) -> None:
    ...
    with respx.mock() as mock:
        ...
        with SyncForecastClient(**client_kwargs) as client:
            assignment = client.create_assignment(req)
...
def test_create_assignment_rejects_invalid_project_id(
    client_kwargs: dict[str, object],
) -> None:
    req = AssignmentRequest(
        start_date=date(2025, 1, 1),
    ...
    with SyncForecastClient(**client_kwargs) as client:
        with pytest.raises(ValueError, match="project_id must be >= 1"):
            client.create_assignment(req)


def test_update_assignment(client_kwargs: dict[str, object]) -> None:
    req = AssignmentRequest(
    ...
    with respx.mock() as mock:
        ...
        with SyncForecastClient(**client_kwargs) as client:
            assignment = client.update_assignment(123, req)
    assert assignment.id == 123


def test_update_assignment_rejects_invalid_ids(
    client_kwargs: dict[str, object],
) -> None:
    req = AssignmentRequest(
        start_date=date(2025, 1, 1),
    ...
    with SyncForecastClient(**client_kwargs) as client:
        with pytest.raises(ValueError, match="id must be >= 1"):
            client.update_assignment(0, req)


def test_delete_assignment(client_kwargs: dict[str, object]) -> None:
    with respx.mock() as mock:
        mock.route(method="DELETE", url__startswith=f"{BASE}/assignments/123").mock(
            return_value=httpx.Response(200),
        )
        with SyncForecastClient(**client_kwargs) as client:
            result = client.delete_assignment(123)
    assert result is None


def test_delete_assignment_rejects_zero_id(
    client_kwargs: dict[str, object],
) -> None:
    with SyncForecastClient(**client_kwargs) as client:
        with pytest.raises(ValueError, match="id must be >= 1"):
            client.delete_assignment(0)


# ---------- meta endpoints ----------


def test_whoami(client_kwargs: dict[str, object]) -> None:
    with respx.mock() as mock:
...
def test_get_account(client_kwargs: dict[str, object]) -> None:
    with respx.mock() as mock:
...
def test_get_subscription(client_kwargs: dict[str, object]) -> None:
    with respx.mock() as mock:
...
def test_remaining_budgeted_hours(client_kwargs: dict[str, object]) -> None:
    with respx.mock() as mock:
...
def test_future_scheduled_hours(client_kwargs: dict[str, object]) -> None:
    with respx.mock() as mock:
        mock.route(
            method="GET",
        ...
        with SyncForecastClient(**client_kwargs) as client:
            items = client.future_scheduled_hours("2025-01-01")
    assert len(items) == 1
    assert items[0].allocation == 8.0


def test_future_scheduled_hours_with_date_object(
    client_kwargs: dict[str, object],
) -> None:
    with respx.mock() as mock:
        mock.route(
        ...
        with SyncForecastClient(**client_kwargs) as client:
            items = client.future_scheduled_hours(date(2025, 1, 1))
    assert len(items) == 1


def test_future_scheduled_hours_for_project(
    client_kwargs: dict[str, object],
) -> None:
    with respx.mock() as mock:
        mock.route(
        ...
        with SyncForecastClient(**client_kwargs) as client:
            items = client.future_scheduled_hours_for_project("2025-01-01", 10)
    assert len(items) == 1
    assert items[0].project_id == 10


def test_assigned_people(client_kwargs: dict[str, object]) -> None:
    with respx.mock() as mock:
        mock.route(
            method="GET",
        ...
        with SyncForecastClient(**client_kwargs) as client:
            result = client.assigned_people("2025-01-01", "2025-01-31")
    assert result["1"] == [2, 3]
    assert result["4"] == [5, 6]


def test_project_heatmap(client_kwargs: dict[str, object]) -> None:
    with respx.mock() as mock:
        ...
        with SyncForecastClient(**client_kwargs) as client:
            items = client.project_heatmap("2025-01-01", "2025-01-31", 10)
    assert len(items) == 1


def test_person_heatmap(client_kwargs: dict[str, object]) -> None:
    with respx.mock() as mock:
        ...
        with SyncForecastClient(**client_kwargs) as client:
            items = client.person_heatmap("2025-01-01", "2025-01-31", 5)
    assert len(items) == 1
    assert items[0].daily_allocation == 480


def test_placeholder_heatmap(client_kwargs: dict[str, object]) -> None:
    with respx.mock() as mock:
        ...
        with SyncForecastClient(**client_kwargs) as client:
            items = client.placeholder_heatmap("2025-01-01", "2025-01-31", 3)
    assert len(items) == 1
...
def test_retries_on_429_then_succeeds(
    ...
    with respx.mock() as mock:
        ...
        with SyncForecastClient(**kwargs) as client:
            people = client.list_people()
    assert len(people) == 1
    assert route.call_count == 2
...
def test_4xx_not_retried(client_kwargs: dict[str, object], fast_retry: RetryPolicy) -> None:
    kwargs = {**client_kwargs, "retry": fast_retry}
    with respx.mock() as mock:
        route = mock.route(method="GET", url__startswith=f"{BASE}/people").mock(
...
def test_401_raises_auth_error(client_kwargs: dict[str, object]) -> None:
    with respx.mock() as mock:
        mock.route(method="GET", url__startswith=f"{BASE}/people").mock(
...
def test_404_raises_not_found_error(client_kwargs: dict[str, object]) -> None:
    with respx.mock() as mock:
        mock.route(method="GET", url__startswith=f"{BASE}/people/999").mock(
            return_value=httpx.Response(404, json={"errors": ["not found"]}),
        )
...
def test_400_raises_validation_error(client_kwargs: dict[str, object]) -> None:
    with respx.mock() as mock:
        mock.route(method="GET", url__startswith=f"{BASE}/assignments").mock(
            return_value=httpx.Response(
...
def test_async_context_manager(client_kwargs: dict[str, object]) -> None:
    with respx.mock() as mock:
        mock.route(method="GET", url__startswith=f"{BASE}/people").mock(
            return_value=httpx.Response(200, json={"people": [_person_data()], "links": {}}),
        )
...
def test_paginate_follows_links_next(client_kwargs: dict[str, object]) -> None:
...
def test_paginate_detects_loop(client_kwargs: dict[str, object]) -> None:
